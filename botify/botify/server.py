...
catalog_own = Catalog(app).load(
    app.config["OWN_TRACKS_CATALOG"], app.config["TOP_TRACKS_CATALOG"], app.config["TRACKS_WITH_DIVERSE_RECS_CATALOG"]
)
...
catalog_own.upload_tracks(tracks_redis.connection, tracks_with_diverse_recs_redis.connection)
catalog_own.upload_artists(artists_redis.connection)
catalog_own.upload_recommendations(recommendations_redis.connection)
catalog_own.upload_recommendations(recommendations_ub_redis.connection, "RECOMMENDATIONS_UB_FILE_PATH")
...
class NextTrack(Resource):
    def post(self, user: int):
        ...
        if not user_history:
            history[user] = {args.track: args.time}
        else:
            user_history[args.track] = args.time
        ...
        if treatment == Treatment.T1:
            recommender = ContextualOwn(tracks_with_diverse_recs_redis.connection, catalog, history)
            #recommender = Contextual(tracks_with_diverse_recs_redis.connection, catalog_own)
        else:
